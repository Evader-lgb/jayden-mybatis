...
public class XMLMapperBuilder extends BaseBuilder {
  ...
  public void parse() {
    ...
    if (!configuration.isResourceLoaded(resource)) {
      // TODO：解析 <mapper namespace="com.jayden.mapper.EmployeeMapper">
   ...
   *    </resultMap>
   *
   *    <select resultMap="result" id="selectById">
   *      <where>
   *        <if test="id>0">
   *            and id=#{id}
   *        </if>
  ...
  private void configurationElement(XNode context) {
    try {
       ...
       * 解析namespace属性
       *
       * <mapper namespace="com.jayden.mapper.EmployeeMapper">
       */
       ...
       * 解析缓存引用
       *
       * 说明当前的缓存引用和UserMapper的缓存引用一致
       * <cache-ref namespace="com.jayden.mapper.UserMapper"></cache-ref>
       *
       * 解析到configuration的cacheRefMap<当前namespace,ref-namespace>
       */
       ...
       * 因为二者是同一引用，在构建XMLMapperBuilder 时把Configuration.getSqlFragments传进去了
       */
       ...
       */
      // TODO 解析到org.apache.ibatis.session.configuration#mappedStatements
  ...
  /**
   * 解析select|update|delte|insert节点，构建mapperStatement对象
   * @param list:所有的select|update|delte|insert节点
   * @param requiredDatabaseId:判断有没有数据库厂商Id
   */
  private void buildStatementFromContext(List<XNode> list, String requiredDatabaseId) {
    // 遍历mapper.xml中所有的select|update|delete|insert节点
    for (XNode context : list) {
      // 创建XMLStatementBuilder对象
      // TODO XMLStatementBuilder
      final XMLStatementBuilder statementParser = new XMLStatementBuilder(configuration, builderAssistant, context, requiredDatabaseId);
      try {
        // TODO：解析各个节点并创建MappedStatement对象的入口
  ...
  /**
   * 处理缓存引用节点
   * <cache-ref namespace="com.jayden.mapper.DeptMapper"></cache-ref>
   */
  private void cacheRefElement(XNode context) {
    if (context != null) {
       ...
       * Key:com.jayden.mapper.User,value:com.jayden.mapper.UserMapper
       */
  ...
  private void cacheElement(XNode context) {
    if (context != null) {
      // TODO：解析cache节点的type属性，不设置则使用默认缓存实现类PerpetualCache
